...
//
//    }
    public function getNumer($id){
        if(!$id){
            return false;
        }
        return $this->field('id,card_number')->where('id='.$id)->select();
    }
public function getNumberInfo($number){
    if(!$number){
        return false;
    }
    return $this->field('id,name')->where('id='.$number)->select();
}

 public   function memberStaff($card_number){
        if(!$card_number){
            return false;
        }
        return $this->where('card_number='.$card_number)->field('id,balance,cardnumber_no,card_number,name')->select();
    }
